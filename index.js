...
module.exports = transformTools.makeStringTransform('moonshinerify', {},
	function (content, transformOptions, done) {
		...
		co(wrapGenerator.mark(function() {
            ...
            return wrapGenerator(function($ctx0) {
                ...
                case 0:
                    $ctx0.next = 2;
                    return openTemp('moonshinerify');
                case 2:
                    tempfile = $ctx0.sent;
                    fs.write(tempfile.fd, content);
                    $ctx0.next = 6;
                    return close(tempfile.fd);
                case 6:
                    $ctx0.next = 8;
                    return openTemp('moonshinerify');
                case 8:
                    outtemp = $ctx0.sent;
                    $ctx0.next = 11;
                    return exec('luac -o ' + outtemp.path + ' ' + tempfile.path);
                case 11:
                    $ctx0.next = 13;
                    return readFile(outtemp.path, 'binary');
                case 13:
                    bytecode = $ctx0.sent.toString();
                    temp.cleanup();
